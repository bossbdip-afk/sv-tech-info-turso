...
def _write_upload_transaction(conn, rows, import_meta):
    ...
    diag['dedupe_seconds'] = round(time.perf_counter() - diag_started, 3)
    ...
    diag['existing_select_decode_seconds'] = round(time.perf_counter() - stage_started, 3)
    stage_started = time.perf_counter()
    ...
    diag['merge_compare_encode_seconds'] = round(time.perf_counter() - stage_started, 3)
    stage_started = time.perf_counter()

    statements = 0
    for start in range(0, len(to_write), 1500):
        statements += _upload_insert_batch(conn, to_write[start:start + 1500])
    diag['upsert_execute_seconds'] = round(time.perf_counter() - stage_started, 3)
    stage_started = time.perf_counter()
    ...
    diag['import_audit_execute_seconds'] = round(time.perf_counter() - stage_started, 3)
    diag['transaction_body_seconds'] = round(time.perf_counter() - diag_started, 3)
    return {
        ...
        'db_stage_timings': diag,
    }


def write_rows(rows, item, import_meta=None):
    write_started = time.perf_counter()
    stage_started = write_started
    conn = connect_item(item, ensure=False)
    connect_seconds = time.perf_counter() - stage_started
    try:
        for attempt in range(2):
            stage_started = time.perf_counter()
            _ensure_upload_schema(conn, item, force=bool(attempt))
            schema_seconds = time.perf_counter() - stage_started
            try:
                # Diagnostic timings only: preserve the exact transaction/data logic.
                stage_started = time.perf_counter()
                conn.execute('BEGIN IMMEDIATE')
                begin_seconds = time.perf_counter() - stage_started
                result = _write_upload_transaction(conn, rows, import_meta)
                stage_started = time.perf_counter()
                conn.commit()
                commit_seconds = time.perf_counter() - stage_started
                result.setdefault('db_stage_timings', {}).update({
                    'connect_seconds': round(connect_seconds, 3),
                    'schema_check_seconds': round(schema_seconds, 3),
                    'begin_seconds': round(begin_seconds, 3),
                    'commit_seconds': round(commit_seconds, 3),
                    'write_rows_total_seconds': round(time.perf_counter() - write_started, 3),
                })
                return result
            except Exception as exc:
                conn.rollback()
...
def _pdf_import_worker(operation, data, district, upazila, filename, item=None, uploaded_by=''):
    ...
    try:
        ...
        result['backend_version'] = '21.23-DIAG'
        return result
    ...
    try:
        ...
        result = {
            ...
            **written, 'records_accounted_unique': accounted,
            ...
            'raw_preserved': raw, 'created_at': now, 'uploaded_by': uploaded_by,
            'preview_cache_hit': cache_hit, 'upload_db_seconds': database_seconds,
            'parser': parser_name, 'backend_version': '21.23-DIAG',
        }
